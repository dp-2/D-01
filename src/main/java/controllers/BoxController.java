...
public class BoxController extends AbstractController {
	...
	public ModelAndView list(@RequestParam(required = false) final Integer parentId) {
		...
		if (parentId == null) {
			...
			final Box parent = this.boxService.findOne(parentId);
			boxs = this.boxService.findByActorAndRoot(actor, parent);
			if (!parent.getRootBox().equals(parent))
				res.addObject("backBoxId", parent.getRootBox().getId());
